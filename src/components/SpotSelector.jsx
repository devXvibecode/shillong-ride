            ...
            >
              <div className="max-w-4xl mx-auto flex items-center gap-3 sm:gap-4 flex-wrap">
                <div className="flex items-center gap-2 flex-1 min-w-0 overflow-x-auto scrollbar-hide">
                  {selectedSpots.map(spotId => {
                    ...
                    return p ? (
                      <span
                        key={spotId}
                        className="inline-flex items-center gap-1 px-3 py-1.5 bg-orange-500/15 text-orange-400 border border-orange-500/30 text-xs sm:text-sm font-['Anton'] uppercase tracking-wider whitespace-nowrap rounded-lg"
                      >
                        {p.name}
                        <button
                          type="button"
                ...
                <div className={`text-xs sm:text-sm font-['Anton'] tracking-wider whitespace-nowrap ${
                  selectedSpots.length === 4 ? 'text-green-400' : 'text-white/55'
                }`}>
                  {selectedSpots.length}/4
                </div>
                <button
                  type="button"
                  onClick={() => setStep(2)}
                  className="glass-btn-primary px-6 py-2.5 text-sm tracking-widest flex-shrink-0"
                >
                  Continue →
                </button>
              </div>
