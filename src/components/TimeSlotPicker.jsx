...
export default function TimeSlotPicker() {
  ...
  return (
    <div className="py-12">
      <h2 className="text-4xl sm:text-6xl font-anton mb-12 text-center">When should we <span className="text-orange-500 text-stroke">Start?</span></h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-5xl mx-auto">
        {SLOTS.map((s, i) => (
          ...
          >
            <div className="text-6xl mb-6 group-hover:scale-125 transition-transform">{s.icon}</div>
            <h3 className="font-anton text-2xl mb-2 uppercase">{s.label}</h3>
            <p className="font-black text-orange-500 text-sm tracking-widest">{s.time}</p>
          </motion.button>
        ))}
      </div>
