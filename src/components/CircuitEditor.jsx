...
function PlaceRow({ place, idx, onEdit, onRemove, onManageImages }) {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-3 p-2.5 bg-white border-4 border-black group hover:shadow-[4px_4px_0px_#000] transition-all"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span className="text-black/30 text-[9px] font-mono">·</span>
          <span className={`text-[9px] font-mono px-1.5 py-0.5 border-2 border-black ${
        ...
        <AnimatePresence>
          {showActions && (
            <motion.div
              ...
              className="absolute right-0 top-full mt-1 z-30 w-44 bg-white border-4 border-black shadow-[6px_6px_0px_#000] overflow-hidden"
            >
              <button
                type="button"
    ...
    >
      ...
      <div className="space-y-3">
        <div>
          ...
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="E.g. Sunset Point" className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
        ...
        <div>
          ...
          <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="Describe this destination..." rows={2} className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold resize-none" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-black/50 text-[10px] font-['Anton'] uppercase tracking-wider mb-1">Category *</label>
            <select value={category} onChange={e => setCategory(e.target.value)} className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold appearance-none">
          ...
          <div>
            ...
            <input type="number" value={distance} onChange={e => setDistance(e.target.value)} placeholder="km" min="1" className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
        ...
        <div>
          ...
          <input type="number" value={price} onChange={e => setPrice(e.target.value)} placeholder="₹" min="0" className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
        </div>
        <div className="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-3 mt-2">
    ...
    <Modal open={true} onClose={() => setEditingPlace(null)} title={`Edit: ${editingPlace.name}`}>
      <div className="space-y-3">
        <div>
          ...
          <input type="text" value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
        </div>
        <div>
          <label className="block text-black/50 text-[10px] font-['Anton'] uppercase tracking-wider mb-1">Description</label>
          <textarea value={editForm.description} onChange={e => setEditForm(f => ({ ...f, description: e.target.value }))} rows={2} className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold resize-none" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-black/50 text-[10px] font-['Anton'] uppercase tracking-wider mb-1">Distance (km)</label>
            <input type="number" value={editForm.distanceWeight} onChange={e => setEditForm(f => ({ ...f, distanceWeight: e.target.value }))} min="1" className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
            <p className="text-black/35 text-[9px] font-mono mt-0.5">Fuel: ₹{Number(editForm.distanceWeight) * 10}/km</p>
          </div>
          <div>
            <label className="block text-black/50 text-[10px] font-['Anton'] uppercase tracking-wider mb-1">Entry Price (₹)</label>
            <input type="number" value={editForm.price} onChange={e => setEditForm(f => ({ ...f, price: e.target.value }))} min="0" className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
          </div>
        </div>
        <div>
          <label className="block text-black/50 text-[10px] font-['Anton'] uppercase tracking-wider mb-1">Category</label>
          <select value={editForm.category} onChange={e => setEditForm(f => ({ ...f, category: e.target.value }))} className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold appearance-none">
            {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div className="flex gap-2 pt-2">
  ...
  return (
    <>
      <Modal open={true} onClose={onClose} title={`Edit Circuit: ${circuit.shortName}`} subtitle={`${circuitSpots.length} spots in this circuit`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="space-y-2 mb-4 max-h-[400px] overflow-y-auto pr-1">
              {circuitSpots.length === 0 ? (
                <div className="text-center py-8 border-4 border-black shadow-[4px_4px_0px_#000] bg-white">
            ...
            {circuitSpots.length > 0 && (
              <div className="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-3">
          ...
          <div className="w-full lg:w-72 flex-shrink-0">
            ...
            <div className="relative mb-2">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-xs border-4 border-black shadow-[3px_3px_0px_#000] font-bold"
              />
            </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between mt-6 pt-4 border-t-4 border-black">
...
function ImageManagerEmbed({ place, onClose }) {
  ...
  return (
    <Modal open={true} onClose={onClose} title={`Images: ${place.name}`} subtitle="Add, remove, or set primary images">
      ...
      {images.length > 0 && (
        <div className="flex gap-2 mb-4">
          <button type="button" onClick={handleReset} className="px-3 py-1.5 bg-red-500 text-white border-2 border-black font-bold text-xs hover:shadow-[3px_3px_0px_#000] transition-all">Reset to Defaults</button>
        </div>
      )}
