                ...
                {expandedId === booking.id && (
                  ...
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                      <div>
                        <p className="text-white/50 text-xs mb-1">Circuit</p>
                        <p className="text-white font-semibold">{booking.circuitName || booking.circuitId}</p>
                      </div>
                      <div>
                        <p className="text-white/50 text-xs mb-1">Pickup Time</p>
                        <p className="text-white font-semibold">{booking.timeSlot ? booking.timeSlot : 'Scheduled post-booking'}</p>
                      </div>
                      <div>
                        <p className="text-white/50 text-xs mb-1">Pickup Location</p>
                        <p className="text-white font-semibold">{booking.pickupLocation || 'Shillong'}</p>
                      </div>
                      <div>
                        <p className="text-white/50 text-xs mb-1">Spots</p>
                        <p className="text-white font-semibold">{booking.spots.map(getPlaceName).join(', ')}</p>
                      </div>
                      <div>
                        <p className="text-white/50 text-xs mb-1">Route Distance</p>
                        <p className="text-white font-semibold">{booking.priceBreakdown.routeDistance} km</p>
                      </div>
                      <div>
                        <p className="text-white/50 text-xs mb-1">Vehicle</p>
                        <p className="text-white font-semibold uppercase">{booking.vehicleType || 'bike'}</p>
                      </div>
                    </div>

                    <div className="mb-4">
                      <p className="text-white/50 text-xs mb-1">Full Route</p>
                      <p className="text-white/80 text-sm">{booking.route.map(id => getPlaceName(id) || id).join(' → ')}</p>
                    </div>
                        ...
                        <div className="mt-3">
                          ...
                          <div className="flex gap-2">
                            <input
                              ...
                              className="flex-1 px-3 py-1.5 bg-white/10 border border-white/20 rounded-lg text-white text-sm placeholder-white/30 focus:outline-none focus:border-amber-400"
                              onKeyDown={(e) => {
