    ...
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={s.label} className={`${s.bg} border-4 border-black shadow-[6px_6px_0px_#000] p-4 text-center`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div className="bg-white border-4 border-black shadow-[6px_6px_0px_#000] p-5">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm pt-2 border-t-2 border-black"><span className="text-black/60">Avg/Booking</span><span className="text-black font-bold">{stats.total > 0 ? fmtShort(Math.round(stats.totalRevenue / stats.total)) : '₹0'}</span></div>
          </div>
        </div>
        <div className="bg-white border-4 border-black shadow-[6px_6px_0px_#000] p-5">
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={s.label}>
                ...
                <div className="h-2 bg-white border-2 border-black"><div className={`h-full transition-all duration-500 ${s.bar}`} style={{ width: `${s.pct}%` }} /></div>
              </div>
            ))}
          </div>
        </div>
...
function RidersView({ onToast }) {
  ...
  return (
    <div>
      <div className="bg-white border-4 border-black shadow-[6px_6px_0px_#000] p-5 mb-6">
        <p className="text-black/50 text-[10px] uppercase tracking-wider mb-3 font-['Anton']">Add Rider</p>
        <div className="flex flex-col sm:flex-row gap-2">
          <input type="text" value={newName} onChange={e => setNewName(e.target.value)} placeholder="Rider name" className="border-4 border-black shadow-[3px_3px_0px_#000] px-3 py-2 text-sm font-bold flex-1"
      ...
      <div className="space-y-2">
        {riders.length === 0 ? (
          ...
          <div key={r.id} className="bg-white border-4 border-black shadow-[6px_6px_0px_#000] p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`w-3 h-3 border-2 border-black ${r.active ? 'bg-green-500' : 'bg-red-500'}`} />
              <div>
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button type="button" onClick={() => { setLog([]); try { localStorage.removeItem('sr_activity_log'); } catch { /* ignore */ } }}
          className="px-3 py-1 text-[10px] font-black uppercase tracking-wider bg-white border-4 border-black shadow-[4px_4px_0px_#000] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all">Clear Log</button>
      </div>
      <div className="space-y-1">
        {log.length === 0 ? (
          <div className="text-center py-12 text-black/30 text-sm font-bold">No activity recorded yet.</div>
        ) : log.map(entry => (
          <div key={entry.id} className="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-3 flex items-center justify-between text-xs">
    ...
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-4">
        {[
          ...
          <div key={s.label} className={`${s.bg} border-4 border-black shadow-[4px_4px_0px_#000] p-3 text-center`}>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name/ID/phone..." className="border-4 border-black shadow-[3px_3px_0px_#000] flex-1 min-w-[160px] max-w-xs px-3 py-1.5 text-xs font-bold" />
        {DATE_FILTERS.map(d => (
          <button key={d.value} type="button" onClick={() => setDateFilter(d.value)}
            className={`px-3 py-1 text-[10px] font-black uppercase tracking-wider border-2 border-black transition-all ${dateFilter === d.value ? 'bg-black text-white shadow-[3px_3px_0px_#000]' : 'bg-white text-black/50'}`}>
            {d.label}
          </button>
        ))}
        ...
        {SORT_OPTIONS.map(s => (
          <button key={s.value} type="button" onClick={() => setSortBy(s.value)}
            className={`px-3 py-1 text-[10px] font-black uppercase tracking-wider border-2 border-black transition-all ${sortBy === s.value ? 'bg-black text-white shadow-[3px_3px_0px_#000]' : 'bg-white text-black/50'}`}>
            {s.label}
          </button>
        ))}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((booking, i) => (
            <motion.div key={booking.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }} className="bg-white border-4 border-black shadow-[6px_6px_0px_#000] overflow-hidden">
              ...
              {expandedId === booking.id && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="border-t-4 border-black px-4 py-4 bg-white">
                  ...
                  <div className="border-t-4 border-yellow-500 pt-3 mb-3">
                    ...
                    <div className="flex justify-between text-sm pt-2 border-t-2 border-black mt-2"><span className="text-black font-bold">Total</span><span className="text-black font-extrabold">{fmt(getRevenue(booking))}</span></div>
                  </div>
                  ...
                  <div className="pt-3 border-t-2 border-black">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {booking.status === 'pending' && (
                        <>
                          <button type="button" onClick={() => updateStatus(booking.id, 'approved')} className="px-3 py-1.5 text-[10px] font-black uppercase tracking-wider bg-white border-2 border-black hover:shadow-[3px_3px_0px_#000] transition-all">Approve</button>
                          <button type="button" onClick={() => updateStatus(booking.id, 'rejected')} className="px-3 py-1.5 text-[10px] font-black uppercase tracking-wider bg-red-500 text-white border-2 border-black hover:shadow-[3px_3px_0px_#000] transition-all">Reject</button>
                        </>
                      )}
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3 border-t-2 border-black pt-3">
                      <button type="button" onClick={() => onWhatsAppOpen?.(booking)}
                        className="px-3 py-1.5 text-[10px] font-black uppercase tracking-wider bg-green-500 text-white border-2 border-black hover:shadow-[3px_3px_0px_#000] transition-all flex items-center gap-1.5">
                    ...
                    {booking.status === 'approved' && (
                      <div className="mb-3">
                        ...
                        <input type="text" placeholder="Rider name..." defaultValue={booking.rider || ''} className="border-4 border-black shadow-[3px_3px_0px_#000] max-w-xs px-3 py-1.5 text-xs font-bold"
                    ...
                    <div className="pt-3 border-t-2 border-red-500">
                      <button type="button" onClick={() => onDeleteBooking(booking.id)} className="px-3 py-1.5 text-[10px] font-black uppercase tracking-wider bg-red-500 text-white border-2 border-black hover:shadow-[3px_3px_0px_#000] transition-all">Delete</button>
                    </div>
                  </div>
                </motion.div>
              )}
        ...
        {circuits.map((circuit, i) => {
          ...
          return (
            <motion.button key={circuit.id} type="button" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
              onClick={() => onCircuitEdit(circuit)} className="bg-white border-4 border-black shadow-[6px_6px_0px_#000] p-5 text-left group hover:shadow-[10px_10px_0px_#000] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-3 h-3 border-2 border-black" style={{ backgroundColor: circuit.color }} />
...
const CATEGORY_COLORS = {
  waterfall: 'text-blue-600 border-black bg-blue-100',
  viewpoint: 'text-purple-600 border-black bg-purple-100',
  lake: 'text-cyan-600 border-black bg-cyan-100',
  cave: 'text-yellow-600 border-black bg-yellow-100',
  village: 'text-green-600 border-black bg-green-100',
  river: 'text-teal-600 border-black bg-teal-100',
...
function CatalogSpotRow({ place, idx, onEdit, onRemove }) {
  ...
  const colorClass = CATEGORY_COLORS[place.category] || 'text-black/50 border-black bg-white';

  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      className="bg-white border-4 border-black shadow-[6px_6px_0px_#000] p-3 sm:p-4 flex items-center gap-3 group"
    >
      <span className="font-['Anton'] text-black/30 text-xs w-6 text-center flex-shrink-0">{idx + 1}</span>
      <div className="w-12 h-12 border-2 border-black flex-shrink-0 bg-cover bg-center" style={{ backgroundImage: `url(${imgUrl})` }} />
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          ...
          <span className="text-black/30 text-[9px]">·</span>
          <span className={`text-[9px] font-mono px-1.5 py-0.5 border-2 border-black ${colorClass}`}>{place.category}</span>
      ...
      <div className="flex gap-1.5 flex-shrink-0">
        ...
        <button type="button" onClick={() => onRemove(place)}
          className="w-7 h-7 flex items-center justify-center bg-white border-2 border-black hover:bg-red-500 text-black/60 hover:text-white transition-all">
...
function SpotEditForm({ place, onSave, onCancel }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
      className="bg-white border-4 border-black shadow-[6px_6px_0px_#000] p-5 mb-4">
      ...
      <div className="space-y-3">
        <div>
          ...
          <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="E.g. Sunset Point" className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
        ...
        <div>
          ...
          <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Describe this destination..." rows={2} className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold resize-none" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-black/50 text-[10px] font-['Anton'] uppercase tracking-wider mb-1">Category *</label>
            <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold appearance-none">
              {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-black/50 text-[10px] font-['Anton'] uppercase tracking-wider mb-1">Distance (km) *</label>
            <input type="number" value={form.distanceWeight} onChange={e => setForm(f => ({ ...f, distanceWeight: e.target.value }))} placeholder="km" min="1" className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
          </div>
        </div>
        <div>
          <label className="block text-black/50 text-[10px] font-['Anton'] uppercase tracking-wider mb-1">Entry Price (₹) *</label>
          <input type="number" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} placeholder="₹" min="0" className="w-full px-3 py-2 text-sm border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
        </div>
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <div className="relative flex-1 min-w-[160px] max-w-xs">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search spots..." className="w-full pl-9 pr-3 py-1.5 text-xs border-4 border-black shadow-[3px_3px_0px_#000] font-bold" />
        </div>
        <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="border-4 border-black shadow-[3px_3px_0px_#000] px-3 py-1.5 text-xs font-bold appearance-none">
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className={`px-2 py-0.5 text-[9px] font-black uppercase tracking-wider border-2 border-black ${
              ...
              <span className={`w-3 h-3 border-2 border-black ${isOnline ? 'bg-green-500' : 'bg-red-500'}`} title={isOnline ? 'Online' : 'Offline'} />
            </div>
            <div className="flex items-center gap-2">
              <button type="button" onClick={handleManualSync} className="bg-white border-4 border-black shadow-[4px_4px_0px_#000] px-3 py-2 text-xs font-black uppercase tracking-wider flex items-center gap-1 hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all">
              ...
              <button type="button" onClick={exportCSV} className="bg-white border-4 border-black shadow-[4px_4px_0px_#000] px-4 py-2 text-xs font-black uppercase tracking-wider hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all">CSV</button>
        ...
        <div className="flex gap-1 mb-6 flex-wrap">
          {[
            ...
            <button key={t.id} type="button" onClick={() => setTab(t.id)}
              className={`px-4 py-2 text-xs font-black uppercase tracking-wider border-4 border-black transition-all ${
                tab === t.id
                  ? 'bg-black text-white shadow-[4px_4px_0px_#000]'
                  : 'bg-white text-black hover:shadow-[4px_4px_0px_#000] hover:translate-x-[-2px] hover:translate-y-[-2px]'
              }`}>
              {t.label}
            </button>
          ))}
